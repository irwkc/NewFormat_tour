...
- ✅ Модерация экскурсий партнеров:
  ...
  - Установка типа процента промоутера (процент или фиксированная сумма) и его значения
  - Процент промоутера можно изменять даже если часть билетов уже продана
...
- ✅ Просмотр баланса в личном кабинете:
  ...
  - Сумма пополнения зависит от типа процента промоутера экскурсии:
...
- ✅ Способы продажи:
  ...
  - **Эквайринг**: создание продажи и последующая загрузка фото чека (аналогично менеджерам)
- ✅ Просмотр своих продаж
...
// Процент промоутера для продавцов
...
**Важно:**
- Tour — это именно «продукт» (маршрут, компания, правила цен и процента промоутера), в нём НЕТ дат/времени/мест.
...
- `POST /api/tours/:id/moderate` - модерация экскурсии (только владелец)
  ...
  - Устанавливает минимальные цены владельца, тип процента промоутера и его значение
...
- `PATCH /api/tours/:id/commission` - изменение процента промоутера экскурсии (только владелец)
...
- `PATCH /api/tours/:id` - обновление экскурсии недоступно (партнер не может ничего изменять после создания)
  ...
  - Владелец может изменять только минимальные цены и процент промоутера через отдельные endpoints
...
- `POST /api/tickets/:id/confirm` - подтверждение использования билета (для контролера)
  ...
  - **Обновляет балансы** у продавца билета:
    - Определяется продавец и тип процента промоутера экскурсии
...
- Владелец модерирует экскурсию:
  ...
  - Устанавливает тип процента промоутера (процент или фиксированная сумма) и его значение
  - Одобряет или отклоняет экскурсию
- После модерации партнер не может ничего изменять в экскурсии
- Владелец может изменять минимальные цены и процент промоутера даже если часть билетов уже продана
...
- Владелец может изменять только минимальные цены и процент промоутера:
  - Минимальные цены: через endpoint `PATCH /api/tours/:id/min-prices`
  - Процент промоутера: через endpoint `PATCH /api/tours/:id/commission`
  - Можно изменить даже если часть билетов уже продана
...
#### Обновление баланса (`balance`) при подтверждении билета (`POST /api/tickets/:id/confirm`):
  - Определяется продавец билета и тип процента промоутера экскурсии
  - Сумма пополнения баланса:
    - Если `commission_type = 'percentage'`: `total_amount` × `commission_percentage` / 100
    - Если `commission_type = 'fixed'`: `commission_fixed_amount`
  - **Записывается история баланса** (`balance_history`):
    ...
    - Сумма: рассчитанная сумма процента промоутера
    - Описание: "Пополнение от продажи билета #ticket_id, сумма продажи: total_amount₽, процент промоутера: amount₽"
...
- **Записывается история баланса** для обоих типов балансов:
  - Для баланса (`balance`): тип `balance`, операция `credit`, описание "Пополнение от продажи билета #ticket_id, сумма продажи: total_amount₽, процент промоутера: amount₽"
...
- **Записывается история баланса** для обоих пользователей:
  - Для баланса промоутера: тип `balance`, операция `credit`, описание "Пополнение от продажи билета #ticket_id за промоутера, сумма продажи: total_amount₽, процент промоутера: amount₽"
...
## Интеграция с внешними партнёрами (задел на будущее)

### Акваэкскурс и подобные компании (со своим сайтом)

**Контекст:** Партнёр (например, Акваэкскурс) ведёт продажи на своём сайте и нуждается в синхронизации с нашей системой.

**Варианты интеграции:**

1. **API для партнёра**
   - Партнёр вызывает наш API для создания броней/продаж
   - Эндпоинты (предположительно):
     - `POST /api/external/bookings` — создание брони (с API-ключом партнёра)
     - `GET /api/external/flights` — список рейсов и доступных мест
   - Модель: `ExternalApiKey` (partner_id, key_hash, permissions), `ExternalBooking` (sale_id, external_id, source)
   - Аутентификация: Bearer API-ключ, привязанный к партнёру

2. **Вебхук / импорт**
   - Наша система получает уведомления о продажах партнёра
   - Эндпоинт: `POST /api/webhooks/partner-sales` (подпись, секрет)
   - Или: импорт CSV/Excel с продажами через владельца
   - Модель: та же `Sale`/`Ticket`, поле `source: 'external'` или `external_booking_id`

3. **Комбинированный вариант**
   - API для проверки мест и создания броней
   - Вебхук для подтверждения оплаты на стороне партнёра

**Рекомендация:** На этапе проектировки выбрать один из вариантов (API или вебхук) по согласованию с партнёром. Реализация — по отдельному запросу.
