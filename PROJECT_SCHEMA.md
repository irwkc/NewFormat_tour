...
- ✅ **При создании автоматически создается помощник владельца** (одна почта, разные пароли)
...
- ✅ **При регистрации автоматически создается альтернативный профиль контролера**
...
- **При создании владельца автоматически создается помощник владельца** с той же почтой, но отдельным паролем
- **При создании партнера автоматически создается альтернативный профиль контролера** с той же почтой, но отдельным паролем
...
### Создание владельца и помощника:
...
2. При создании владельца заполняются:
   ...
   - Пароль для помощника владельца (можно задать или сгенерировать автоматически)
3. **Автоматически создается помощник владельца:**
   - Та же почта (email)
   - Отдельный пароль (заданный при создании)
   - Роль: `owner_assistant`
   - Связь с основным профилем через `main_owner_id`
4. Владелец (основной профиль) может войти по **email + пароль основного профиля**
5. Помощник владельца (альтернативный профиль) может войти по **email + пароль помощника**
6. Пароль помощника владельца можно изменить в настройках владельца
...
3. Партнер заполняет:
   ...
   - Пароль для альтернативного профиля контролера (можно задать или сгенерировать автоматически)
4. Система отправляет письмо с подтверждением email
5. Партнер подтверждает email
6. **Автоматически создается альтернативный профиль контролера:**
   - Та же почта (email)
   - Отдельный пароль (заданный при регистрации)
   - Роль: `partner_controller`
   - Связь с основным профилем через `main_partner_id`
7. Партнер (основной профиль) может войти по **email + пароль основного профиля**
8. Партнер (альтернативный профиль) может войти по **email + пароль контролера**
9. Пароль альтернативного профиля можно изменить в настройках основного профиля
...
Все основные вопросы были отвечены и интегрированы в схему проекта.
