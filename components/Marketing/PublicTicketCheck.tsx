...
type Props = {
  /** На главной — компактный заголовок; на отдельной странице — полная шапка */
  layout?: 'home' | 'page'
  /** id поля ввода (уникальный на странице) */
  inputId?: string
}
...
export function PublicTicketCheck({ layout = 'page', inputId = 'sale-code' }: Props) {
  ...
  const submit = async () => {
    ...
    if (!/^\d{6}$/.test(trimmed)) {
      setError('Введите 6 цифр — номер заказа с чека')
      setData(null)
      return
    }
    setError(null)
  ...
  return (
    <div className={isHome ? '' : 'max-w-lg mx-auto px-3 sm:px-6 py-8 sm:py-12'}>
      {isHome ? (
        <>
          <h2 className="text-xl sm:text-2xl font-bold text-white mb-1 font-[family-name:var(--font-poppins)]">
            Проверка билета
          </h2>
          <p className="text-white/70 text-sm sm:text-base mb-4 sm:mb-5 leading-relaxed max-w-2xl">
            Введите шестизначный номер заказа с чека или из письма.
          </p>
        </>
      ) : (
        <>
          ...
          <p className="text-white/70 text-sm sm:text-base mb-8 leading-relaxed">
            Введите шестизначный номер заказа с чека или из письма — мы покажем экскурсию, дату отправления и статус
            билета.
          </p>
        </>
      )}

      <div className={`glass-card space-y-4 ${isHome ? 'max-w-lg' : ''}`}>
        <div>
          ...
          <input
            ...
            className="input-glass text-center text-xl tracking-[0.35em] font-mono"
          />
        </div>
