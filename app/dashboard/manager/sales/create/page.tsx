    ...
    <DashboardLayout title="Создание продажи" navItems={navItems}>
      <div className="px-4 py-6 sm:px-0">
        <div className="bg-white shadow-md rounded-lg p-6 max-w-2xl mx-auto">
          <h2 className="text-2xl font-bold mb-6">Создать продажу</h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              {(paymentMethod === 'cash' || paymentMethod === 'acquiring') && (
                <div>
                  ...
                  />
                  {promoterInfo && (
                    <p className="text-sm text-green-600 mt-1">Промоутер: {promoterInfo.full_name}</p>
                  )}
                  {errors.promoter_id && (
                    <p className="text-red-500 text-xs mt-1">{errors.promoter_id.message}</p>
                  )}
                </div>
              )}
