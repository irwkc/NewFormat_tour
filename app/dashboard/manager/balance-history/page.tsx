  ...
  const fetchMe = async () => {
    try {
      ...
      if (data.success && data.data) {
        ...
        const freshDebt = Number(data.data.debt_to_company ?? 0)
        setBalance(freshBalance - freshDebt)
        updateUser({ balance: freshBalance, debt_to_company: freshDebt })
      }
    } catch {
      setBalance(null)
    }
    ...
    <DashboardLayout title="История баланса" navItems={navItems}>
      <div className="space-y-6">
        <div className="glass-card">
          ...
          <div className={`text-3xl font-bold ${(balance !== null ? balance : Number(user?.balance ?? 0) - Number(user?.debt_to_company ?? 0)) >= 0 ? 'text-green-300' : 'text-red-300'}`}>
            {(balance !== null ? balance : Number(user?.balance ?? 0) - Number(user?.debt_to_company ?? 0)).toFixed(2)}₽
          </div>
        </div>
